...
/**
 * Implementation of the Codec interface for 
...
public class HTMLEntityCodec extends Codec
{
	...
	public Character decodeCharacter(PushbackString input)
	{
		...
		if(second == '#') { //Handles numbers
			Character c = getNumericEntity( input );
			if ( c != null ) return c;
		} else if ( Character.isLetter( second.charValue() ) ) { //Handles entities
	...
	private Character parseNumber(PushbackString input)
	{
		...
		while(input.hasNext()) {
			...
			if (Character.isDigit(c.charValue())) { //If character is a digit then add it on and keep going
				sb.append( c );
				input.next();
			} else if (c == ';' ) {  //If character is a semi-colon, eat it and quit
				input.next();
				break;
	...
	private Character parseHex( PushbackString input ) 
	{
		...
		{
			Character c = input.peek();
			if ( "0123456789ABCDEFabcdef".indexOf(c) != -1 ) { //If character is a hex digit then add it on and keep going
				sb.append( c );
				input.next();
			} else if (c == ';' ) { //If character is a semi-colon, eat it and quit
				input.next();
