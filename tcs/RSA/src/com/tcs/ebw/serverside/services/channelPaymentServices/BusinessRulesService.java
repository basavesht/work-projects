...
public class BusinessRulesService implements Serializable
{
	...
	private String dont_spawn_flag;
				...
				for(int i=0;i<si_return.getOutputVector().size();i++)
				{
					...
					else if(si_return.getOutputVector().get(i) instanceof BR_VALIDATION_LOG)
					{
						...
						else if(objBr_validation_log.getERR_LVL().equalsIgnoreCase("Warning"))
						{
							...
							//RSA Review flag...
							if(objBr_validation_log.getRSA_REVIEW_FLAG()!=null && !objBr_validation_log.getRSA_REVIEW_FLAG().trim().equalsIgnoreCase("")){
							...
							//Don't spawn flag..
							if(objBr_validation_log.getDONT_SPAWN_FLAG()!=null && !objBr_validation_log.getDONT_SPAWN_FLAG().trim().equalsIgnoreCase("")){
