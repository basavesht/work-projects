...
public class SendNotifications 
{
	...
	public Vector setNotificationDetails(HashMap notifyDetails,ServiceContext serviceContext) throws Exception,SQLException 
	{
		...
		try
		{
			...
			if(notifyDetails.containsKey("PaymentDetails")){
				objPaymentDetails = (PaymentDetailsTO)notifyDetails.get("PaymentDetails");
			}
			...
			if(!isExternalAcc)
			{
				...
				if(isReturnCodeDesc){
					...
					objMo_attr_event_eval.setATTR_VAL(objPaymentDetails.getCase_status());
					objMo_attr_event_eval_vec.add(objMo_attr_event_eval);
				}
				...
				objMo_rcvr_adrs.setRMRKS(objPaymentDetails.getCurrent_owner_role()); //RMRKS is set in case of External Accounts Notifications...
			...
			else 
			{
				...
				//7.Setting the MSSBAccountOwner.
				if(isAcntOwnerMSSB){
					MO_ATTR_EVENT_EVAL objMo_attr_event_eval = new MO_ATTR_EVENT_EVAL();
				...
				//9.LifeTimeUserID mapping for determining EMAIL or ALERT Setup for Transaction case...
				MO_ATTR_EVENT_EVAL objMo_attr_event_eval = new MO_ATTR_EVENT_EVAL();
				objMo_attr_event_eval.setATTR_NM("LifeTimeUserID");
				objMo_attr_event_eval.setATTR_VAL(isLifeUserIdExists(objNotificationOut.getLifeTimeUserId()));
				objMo_attr_event_eval_vec.add(objMo_attr_event_eval);
	...
	public String getNotification_Txn_Type(NotificationOutDetails objNotificationOut) throws Exception
	{
		try 
		{
			...
			if(objNotificationOut.getTxnType()!=null && objNotificationOut.getTxnType().equalsIgnoreCase(TxnTypeCode.INTERNAL)){
