...
public class Sha4J {
	...
	public byte[] sha1Digest(InputStream inputStream) throws IOException {
		...
		for (;;) {
			...
			int t = 0;
			for (; t < 80; t++) {
				int T = modulo32Add(modulo32Add(modulo32Add(ROTL(5, a), f(t, b,
						c, d)), modulo32Add(e, K(t))), W[t]);
				e = d;
				d = c;
				c = ROTL(30, b);
				b = a;
	...
	private void doPadBlock(int[] H, int[] M, int[] W, boolean addOne) {
		...
		int t = 0;
		for (; t < 80; t++) {
			int T = modulo32Add(modulo32Add(modulo32Add(ROTL(5, a), f(t, b, c,
					d)), modulo32Add(e, K(t))), W[t]);
			e = d;
			d = c;
			c = ROTL(30, b);
			b = a;
	...
	private byte[] sha256Digest(InputStream inputStream, boolean sha224)
			throws IOException {
		// read bytes
		byte[] Mb = new byte[64];// 512-bit block
		int[] M = new int[16];
	...
	private void doPadBlock256(int[] H, int[] M, int[] W, boolean addOne) {
		...
		int T2 = 0;
		for (; t < 64; t++) {
			T1 = modulo32Add(modulo32Add(modulo32Add(h, SIGMA256_1(e)),
					modulo32Add(Ch(e, f, g), K256[t])), W[t]);
	...
	private void doPadBlock512(long[] H, long[] M, long[] W, boolean addOne) {
		...
		long T2 = 0;
		for (; t < 80; t++) {
			T1 = modulo64Add(modulo64Add(modulo64Add(modulo64Add(h,
					SIGMA512_1(e)), Ch(e, f, g)), K512[t]), W[t]);
