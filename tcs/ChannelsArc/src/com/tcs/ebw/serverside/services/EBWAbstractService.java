...
public abstract class EBWAbstractService implements IEBWService {
	...
	public Object execute(Class paramTypes[],Object params[]) throws Exception {
		...
		try{
			...
			String processTimeout = System.getProperty("bancs.channels.processTimeout");
			if(processTimeout ==  null){
				retVal = m.invoke(this,params);
			}
			else {
				long timeoutInSeconds = Integer.parseInt(processTimeout); 
				retVal = callMethodWithTimeout(this,m,params, timeoutInSeconds); 
			}
		...
		{
			if ( serviceConnection != null && serviceConnection instanceof java.sql.Connection ){
				try {
					Connection conn  = (java.sql.Connection)serviceConnection;
					conn.rollback();
					conn.close();
					EBWLogger.logDebug(this, "forcefully rolledbacked and closed connection");
				} 
	...
	/**
	 * Invoking the custom service method with reflection and timeout (Configured).
	 * @param service
	 * @param m
	 * @param params
	 * @param timeout
	 ...
	 */
	public static Object callMethodWithTimeout(EBWAbstractService service, Method m, Object[] params, long timeout) throws InvocationTargetException
	{
		...
		try
		{
			Callable<Object> task = new Callable<Object>() {
				EBWAbstractService service;
				Method method;
				Object[] params;
				public Callable<Object> setParams(EBWAbstractService s, Method m, Object[] p){
					service = s;
					method = m;
					params = p;
					return this;
			...
			}.setParams(service,m,params);
			Future<Object> future = executor.submit(task); 
			try {    
				if ( timeout > 0 ){
					retVal = future.get(timeout, TimeUnit.SECONDS);
				} else{
					retVal = future.get();
				}
			} 
			catch (TimeoutException ex) { 
				retVal = new InvocationTargetException(ex);
			} 
		...
		if ( retVal instanceof InvocationTargetException){
