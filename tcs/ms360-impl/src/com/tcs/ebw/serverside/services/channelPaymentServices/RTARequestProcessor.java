...
public class RTARequestProcessor {
	...
	//Transfer types..
	public enum TransferType {INTERNAL,ACH_WITHDRAWAL,ACH_DEPOSIT,CHK_LOCAL,CHK_REGIONAL,PORTFOLIO_LOAN};
	...
	{ 
		...
		try 
		{
			...
			if(objPaymentDetails.isTxnCancelled() || objPaymentDetails.isTxnRejected())
			{
	...
	public static TransferType setTransferType(String transferType)
	{
		...
		if(transferType!=null && transferType.equalsIgnoreCase(TxnTypeCode.INTERNAL)){
			...
			txnType = TransferType.PORTFOLIO_LOAN;
		}
		return txnType;
	}
}
