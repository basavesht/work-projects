...
import com.tcs.ebw.serverside.services.channelPaymentServices.AcntPlatingInterfaceIn;
...
import com.tcs.ebw.serverside.services.channelPaymentServices.CalculatedFee;
import com.tcs.ebw.serverside.services.channelPaymentServices.CheckNumberDetails;
import com.tcs.ebw.serverside.services.channelPaymentServices.CheckNumberInterfaceIn;
import com.tcs.ebw.serverside.services.channelPaymentServices.FeeChargeInterfaceIn;
...
import com.tcs.ebw.serverside.services.channelPaymentServices.PortfolioLoansInterfaceIn;
import com.tcs.ebw.serverside.services.channelPaymentServices.PortfolioLoansService;
import com.tcs.ebw.serverside.services.channelPaymentServices.PrintCheckInterfaceIn;
import com.tcs.ebw.serverside.services.channelPaymentServices.PrintCheckOutDetails;
...
public class PaymentsUtility {
	...
	/**** 
	 * 
	 * Interface : Channel --> SI --> Payments Hub TRIAL_DEPO ( Remittance or DDRemittance )
	 *
	 ****/
	public static void executeTrialDepoPayment(HashMap txnDetails,final ServiceContext context)throws Exception,SQLException
	{
		EBWLogger.trace("PaymentsUtility", "Calling the Payments Hub service thorugh SI for Trial Depo");
		ArrayList objPayOutDetails = new ArrayList();
		try 
		{
			//Interface attributes..
			...
			//Mapping the interface input vectors...
			Vector<Object> paymentsInputVector = PaymentHubInterfaceIn.setPaymentHubParams_TrialDepo(txnDetails,context);
			Vector<Object> paymentsCtrlVector = setInterfaceCntrlVector("MSCHIO","STP",context);
			...
			if(paymentHubSIOutput!=null && !paymentHubSIOutput.isEmpty()){
				objPayOutDetails= PaymentOutDetails.getPaymentOutDetails(paymentHubSIOutput,context);
				txnDetails.put("TrialDepoOuputDetails", objPayOutDetails);
			}
			else {
				context.addMessage(MessageType.CRITICAL,ChannelsErrorCodes.INTERNAL_ERROR);
			}
	...
	/**
	 * 
	 * Interface : Channel --> SI --> BusinessRules Service for Blocked Display Rule
	 * @throws Exception 
	 *
	 ****/

	public static void executeBlockDisplay(HashMap txnDetails,ServiceContext context) throws Exception
	{
		EBWLogger.trace("PaymentsUtility", "Calling the Business Rule Service (Blocked Display) through SI from Channels");
		...
		try 
		{
			...
			//Mapping the interface input vectors...
			Vector<Object> blockDispInputVector = BusinessRuleInterfaceIn.getBRInterfaceBlockDispIn(txnDetails,context);
			...
			if(businessRuleSIOutput!=null && !businessRuleSIOutput.isEmpty()){
				...
				txnDetails.put("BlockedDispOutputDetails", objBusinessRulesService);
			}
			else{
				context.addMessage(MessageType.CRITICAL,ChannelsErrorCodes.INTERNAL_ERROR);
			}
	...
	public static void executeBRTransaction(HashMap txnDetails,final ServiceContext context) throws Exception
	{
		...
		try 
		{
			...
			//Mapping the interface input vectors...
			Vector<Object> brTransactionInputVector = BusinessRuleInterfaceIn.setBRInterfaceTransactionRuleIn(txnDetails,context);
			Vector<Object> brTransactionCtrlVector = setInterfaceCntrlVector("MSCHIO","STP",context);

			//Invoking Interface call...
			JavaAPIHelper.init(CONFIGPATH);
			businessRuleSIOutput = JavaAPIHelper.processMessage(brTransactionInputVector, brTransactionCtrlVector);
			if(businessRuleSIOutput!=null && !businessRuleSIOutput.isEmpty()){
				objBusinessRulesService= BusinessRulesService.getBusinessRuleOutput(txnDetails,businessRuleSIOutput,context);
	...
	/**
	 * 
	 * Interface : Channel --> SI --> BusinessRules Service for Account Rule..
	 * @throws Exception 
	 *
	 ****/

	public static void executeBRAccount(HashMap txnDetails,final ServiceContext context) throws Exception
	{
		EBWLogger.trace("PaymentsUtility", "Calling the Business Rule Service (Account) through SI from Channels");
		BusinessRulesService objBusinessRulesService = new BusinessRulesService();
		try 
		{
			//Interface attributes..
			...
			//Mapping the interface input vectors...
			Vector<Object> brAccountInputVector = BusinessRuleInterfaceIn.setBRInterfaceAccountRuleIn(txnDetails,context);
			Vector<Object> brAccountCtrlVector = setInterfaceCntrlVector("MSCHIO","STP",context);

			//Invoking Interface call...
			JavaAPIHelper.init(CONFIGPATH);
			businessRuleSIOutput = JavaAPIHelper.processMessage(brAccountInputVector, brAccountCtrlVector);
			if(businessRuleSIOutput!=null && !businessRuleSIOutput.isEmpty()){
				objBusinessRulesService= BusinessRulesService.getBusinessRuleOutput(txnDetails,businessRuleSIOutput,context);
				txnDetails.put("BROutputDetails", objBusinessRulesService);
			}
	...
	/**** 
	 * 
	 * Interface : Channel --> SI --> Account Plating
	 *
	 ****/
	public static void getAccountPlatingInfo(HashMap txnDetails,final ServiceContext context)throws Exception,SQLException
	{
		EBWLogger.trace("PaymentsUtility", "Starting getAccountPlatingInfo method in PaymentsUtility class");
		AccountPlating objAccPlatingDetails = new AccountPlating();
		try 
		{
			//Interface attributes..
			Vector<Object> accPlatingSIOutput = null;

			//Mapping the interface input vectors...
			Vector<Object> accPlatingInputVector = AcntPlatingInterfaceIn.setAcntPlatingInputDetails(txnDetails,context);
			Vector<Object> accPlatingCtrlVector = setInterfaceCntrlVector("MSCHIO","AccountPlating",context);

			//Invoking Interface call...
			JavaAPIHelper.init(CONFIGPATH);
			accPlatingSIOutput = JavaAPIHelper.processMessage(accPlatingInputVector, accPlatingCtrlVector);
			if(accPlatingSIOutput!=null && !accPlatingSIOutput.isEmpty()){
				objAccPlatingDetails = AccountPlating.getAcntPlatingInfo(accPlatingSIOutput,context);
				txnDetails.put("AcntPlatingOutputDetails",objAccPlatingDetails);
			}
			else{
				context.addMessage(MessageType.CRITICAL,ChannelsErrorCodes.INTERNAL_ERROR);
			}
		...
		EBWLogger.trace("PaymentsUtility", "Finished getAccountPlatingInfo method in PaymentsUtility class");
	}

	/**** 
	 * 
	 * Interface : Channel --> SI --> FEE Charge 
	 *
	 ****/
	public static void getFeeCharge(HashMap txnDetails,final ServiceContext context)throws Exception,SQLException
	{
		EBWLogger.trace("PaymentsUtility", "Starting getFeeCharge method in PaymentsUtility class");
		try 
		{
			//Interface attributes..
			Vector<Object> feeChargeSIOutput = null;

			//Mapping the interface input vectors...
			Vector<Object> feeChargeInpVector = FeeChargeInterfaceIn.setFeeChargeDetails(txnDetails,context);
			Vector<Object> feeChargeCntrlVector = setInterfaceCntrlVector("MSCHIO","STP",context);

			//Invoking Interface call...
			JavaAPIHelper.init(CONFIGPATH);
			feeChargeSIOutput = JavaAPIHelper.processMessage(feeChargeInpVector, feeChargeCntrlVector);
			if(feeChargeSIOutput!=null && !feeChargeSIOutput.isEmpty()){
				CalculatedFee.getFee(txnDetails,feeChargeSIOutput,context);
			}
			else{
				context.addMessage(MessageType.CRITICAL,ChannelsErrorCodes.INTERNAL_ERROR);
			}
		...
		EBWLogger.trace("PaymentsUtility", "Finished getFeeCharge method in PaymentsUtility class");
	}

	/**** 
	 * 
	 * Interface : Channel --> SI --> Check Number Interface...
	 *
	 ****/
	public static void getCheckNumber(HashMap txnDetails,final ServiceContext context)throws Exception,SQLException
	{
		EBWLogger.trace("PaymentsUtility", "Starting getCheckNumber method in PaymentsUtility class");
		try 
		{
			//Interface attributes..
			Vector<Object> checkNumberSIOutput = null;

			//Mapping the interface input vectors...
			Vector<Object> checkNumberInpVector = CheckNumberInterfaceIn.setCheckNumberInput(txnDetails,context);
			Vector<Object> checkNumberCntrlVector = setInterfaceCntrlVector("MSCHIO","ChqNo",context);

			//Invoking Interface call...
			JavaAPIHelper.init(CONFIGPATH);
			checkNumberSIOutput = JavaAPIHelper.processMessage(checkNumberInpVector, checkNumberCntrlVector);
			if(checkNumberSIOutput!=null && !checkNumberSIOutput.isEmpty()){
				CheckNumberDetails.getCheckNumberDetails(txnDetails,checkNumberSIOutput,context);
			}
			else{
				context.addMessage(MessageType.CRITICAL,ChannelsErrorCodes.INTERNAL_ERROR);
			}
		...
		EBWLogger.trace("PaymentsUtility", "Finished getCheckNumber method in PaymentsUtility class");
	}

	/**** 
	 * 
	 * Interface : Channel --> SI --> Bottom Line Print Check Interface...
	 *
	 ****/
	public static void executeCheckPrint(HashMap txnDetails,final ServiceContext context)throws Exception,SQLException
	{
		EBWLogger.trace("PaymentsUtility", "Starting executeCheckPrint method in PaymentsUtility class");
		try 
		{
			//Interface attributes..
			Vector<Object> checkPrintOutput = null;

			//Mapping the interface input vectors...
			Vector<Object> checkPrintInpVector = PrintCheckInterfaceIn.setPrintCheckInput(txnDetails,context);
			Vector<Object> checkPrintCntrlVector = setInterfaceCntrlVector("MSCHIO","PrintRequest",context);

			//Invoking Interface call...
			JavaAPIHelper.init(CONFIGPATH);
			checkPrintOutput = JavaAPIHelper.processMessage(checkPrintInpVector, checkPrintCntrlVector);
			if(checkPrintOutput!=null && !checkPrintOutput.isEmpty()){
				PrintCheckOutDetails.getPrintCheckOutDetails(txnDetails,checkPrintOutput,context);
			}
			else{
				context.addMessage(MessageType.CRITICAL,ChannelsErrorCodes.INTERNAL_ERROR);
			}
		...
		EBWLogger.trace("PaymentsUtility", "Finished executeCheckPrint method in PaymentsUtility class");
	}

	/**** 
	 * 
	 * Interface : Channel --> SI --> PLA Details ..
	 *
	 ****/
	public static void fetchPLACreditLoans(HashMap txnDetails,final ServiceContext context)throws Exception,SQLException
	{
		EBWLogger.trace("PaymentsUtility", "Calling the PLA Details web service through SI from Channels");
		PortfolioLoansService objPLAService = new PortfolioLoansService();
		try 
		{
			//Interface attributes..
			Vector<Object> plaAccountsSIOutput = null;

			//Mapping the interface input vectors...
			Vector<Object> plaAccountsInputVector = PortfolioLoansInterfaceIn.setPLAAccountsDtls(txnDetails,context);
			Vector<Object> plaAccountsCtrlVector = setInterfaceCntrlVector("MSCHIO","PLAReqst",context);

			//Invoking Interface call...
			JavaAPIHelper.init(CONFIGPATH);
			plaAccountsSIOutput = JavaAPIHelper.processMessage(plaAccountsInputVector, plaAccountsCtrlVector);
			if(plaAccountsSIOutput!=null && !plaAccountsSIOutput.isEmpty()){
				objPLAService = PortfolioLoansService.getPLAAccountsDtls(txnDetails,plaAccountsSIOutput,context);
				txnDetails.put("PLACreditLoans", objPLAService);
			}
			else {
				context.addMessage(MessageType.CRITICAL,ChannelsErrorCodes.INTERNAL_ERROR);
			}
