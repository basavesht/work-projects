...
public class UserEntilements extends DatabaseService{
	...
	public void checkIntAccEntitlement(HashMap txnDetails,ServiceContext serviceContext) throws Exception
	{
		...
		try 
		{
			...
			String preNavigationPage = objPaymentDetails.getPreNavigationPage();
			String printingBranch = objCheckRequest.getPrintingBranch();
			...
			for(int i=0; i<eligibleAccounts.size(); i++) 
			{
				...
				if(transferType!=null && (transferType.equalsIgnoreCase(TxnTypeCode.INTERNAL) || transferType.equalsIgnoreCase(TxnTypeCode.ACH_WITHDRAWAL) || transferType.equalsIgnoreCase(TxnTypeCode.PORTFOLIO_LOAN)))
				{
				...
				if(transferType!=null && (transferType.startsWith(ChkReqConstants.CHK)))
				{
					if(objAccessibleAcc.isSuperUser()){
						...
						if(preNavigationPage!=null && preNavigationPage.equalsIgnoreCase(MSSystemDefaults.BPM_DESC)){
							if(objAccessibleAcc.getBranch()== Integer.parseInt(printingBranch)){
								isAccountEligible = true;
								break;
							}
						}
		...
		try
		{
			...
			if(!isExtAccSpokeValid){
				...
				if(auth_mode!=null && auth_mode.equalsIgnoreCase(MSSystemDefaults.AUTH_VERBAL)){
					...
					serviceContext.addMessage(MessageType.CRITICAL,ChannelsErrorCodes.EXT_ACC_SIGNED_BY_FAILURE);
				}
			}
			else {
