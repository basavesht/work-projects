...
public class MultiTransferProcessor extends DatabaseService {
	...
	public static Object[] setCancelTransactionData(HashMap txnDetails,ArrayList txnRefDetails,Date businessDate) throws Exception 
	{
		try 
		{
			...
			//Payment details..
			PaymentDetailsTO objParentTxnDetails = new PaymentDetailsTO();
			if(txnDetails.containsKey("PaymentDetails")){
				objParentTxnDetails = (PaymentDetailsTO)txnDetails.get("PaymentDetails");
			}
			...
			cancelPayDtls.setCreated_by_comments(objParentTxnDetails.getCreated_by_comments());
		...
		try 
		{
			...
			if(transferList!=null && !transferList.isEmpty())
			{
				if(transferList.size()>0)
				{
					...
					{
						//Setting the transaction data...
						Object[] cancelTxnData = setSuspendTransactionData(txnDetails,(ArrayList)transferList.get(j),businessDate);

						//Mapping the Payment details (Internal/External Cancel Transfers)...
						MapPaymentInputDetails objMapConfirmDetails = new MapPaymentInputDetails();
						objMapConfirmDetails.setConnection(serviceConnection);
						suspendTxnDetails = objMapConfirmDetails.setPaymentConfirmDetails(cancelTxnData,serviceContext);
