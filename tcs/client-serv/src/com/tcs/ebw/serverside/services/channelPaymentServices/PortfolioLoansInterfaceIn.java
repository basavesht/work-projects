...
public class PortfolioLoansInterfaceIn {
	...
	public static Vector<Object> setPLAAccountsDtls(HashMap txnDetails,ServiceContext serviceContext) throws Exception
	{
		...
		try 
		{
			...
			objMS_PLA_INPUTTO.setREQUEST_HOST(WSDefaultInputsMap.getPLARequestHost(new Boolean(true),serviceContext));
			objMS_PLA_INPUTTO.setSUBMIT_TIME(new Timestamp(new Date().getTime()).toString());
			objMS_PLA_INPUTTO.setSIGN_ON_RACF_ID(WSDefaultInputsMap.getPLAUserID(new Boolean(true),serviceContext));
			objMS_PLA_INPUTTO.setAPPL_ID(WSDefaultInputsMap.getPLAAppID(new Boolean(true),serviceContext));
