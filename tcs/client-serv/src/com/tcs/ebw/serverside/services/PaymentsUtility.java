...
public class PaymentsUtility {
	...
	public static void executeBlockDisplay(HashMap txnDetails,ServiceContext context) throws Exception
	{
		...
		try 
		{
			...
			if(businessRuleSIOutput!=null && !businessRuleSIOutput.isEmpty()){
				objBusinessRulesService= BusinessRulesService.getBusinessRuleOutput(businessRuleSIOutput,context);
	...
	/***
	 * 
	 * Interface : Channel --> SI --> Send Notifications during transfer modify/cancel etc...
	 * @throws Exception 
	 *
	 ****/
		...
		try 
		{
			...
			if(isLimitActionReq)
			{
				...
				if(limitBRSIOut!=null && !limitBRSIOut.isEmpty()){
					BusinessRulesService.getBusinessRuleOutput(limitBRSIOut,context);
				}
				else{
					context.addMessage(MessageType.CRITICAL,ChannelsErrorCodes.INTERNAL_ERROR);
				}
