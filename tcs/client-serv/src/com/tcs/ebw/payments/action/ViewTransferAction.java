/*
 * Created on Fri Jun 05 15:20:07 IST 2009
 *
 ...
 * **********************************************************
 */
public class ViewTransferAction extends EbwAction {
	/**
	...
	/**
	 * performTask method of ViewTransferAction class. 
	 */ 
	public ActionForward performTask (ActionMapping mapping, ActionForm form, 
		...
		try {
			...
			ViewTransferForm objViewTransferForm = (ViewTransferForm) form; 

			UserPrincipal objUserPrincipal = getUserPrincipal(request);

			String action = objViewTransferForm.getAction();
			String state = objViewTransferForm.getState();
			log ("ViewTransferForm Action : " + action);
			log ("ViewTransferForm State  : " + state);

			EbwForm objPostActionForm = null;

			String actionType = objViewTransferForm.getActionType();
			if(actionType!=null && actionType.length()>0 && actionType.equalsIgnoreCase("export")){
				action = "export";
				state = objViewTransferForm.getPrevState();
			}
			request.setAttribute("ViewTransferForm", objViewTransferForm);
			if (action.equals("INIT")) {
				ViewTransferDelegate objViewTransferDelegate = new ViewTransferDelegate(objViewTransferForm, getUserObject(request), objUserPrincipal);
				ViewTransferForm objViewTransferFrm = null;
				if (state.equals("ViewTransfer_INIT")) {
					objViewTransferFrm = objViewTransferDelegate.viewTransfer_INIT_iNIT();
					objPostActionForm=(EbwForm)objViewTransferFrm;
				}
				else if (state.equals("ViewTransfer_Recurring")) {
					objViewTransferFrm = objViewTransferDelegate.viewTransfer_Recurring_iNIT();
					objPostActionForm=(EbwForm)objViewTransferFrm;
				}
				else if (state.equals("ViewTransfer_OneTimeHistory")) {
					objViewTransferFrm = objViewTransferDelegate.viewTransfer_OneTimeHistory_iNIT();
					objPostActionForm=(EbwForm)objViewTransferFrm;
				}
				else if (state.equals("ViewTransfer_RecurringHistory")) {
					objViewTransferFrm = objViewTransferDelegate.viewTransfer_RecurringHistory_iNIT();
					objPostActionForm=(EbwForm)objViewTransferFrm;
				}
				if(getUserObject(request).get("ViewTransferForm")!=null)
					objViewTransferFrm.copyCollections(((ViewTransferForm)getUserObject(request).get("ViewTransferForm")));
				request.setAttribute("ViewTransferForm", objViewTransferFrm);
				forwardTo = "ViewTransfer";
			} else {
