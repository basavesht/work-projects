...
public class UpdateTxnParentService extends DatabaseService {
	...
	public void setUpdateTxnParentDetails(HashMap txnDetails,ServiceContext serviceContext)throws Exception,SQLException 
	{
		...
		try
		{
			...
			nextBusinessDate = MSCommonUtils.evaluateNextTxnDate(frequencyValue,last_transfer_date,parentTxnDate,transferType,Boolean.TRUE,serviceContext);
			...
			if(isRecurringActive && (dont_spawn_flag==null || dont_spawn_flag.equalsIgnoreCase("N")))
			{
				for (int i= 0; i< nextChildTxnTOobj.length;i++)  
				{
					if(nextChildTxnTOobj[i] instanceof com.tcs.ebw.payments.transferobject.DsBatchTO){
						((DsBatchTO)nextChildTxnTOobj[i]).setBatstatus("6");
					}
					if(nextChildTxnTOobj[i] instanceof com.tcs.ebw.payments.transferobject.DsPayTxnsTO){
						...
						((DsPayTxnsTO)nextChildTxnTOobj[i]).setPayccsstatuscode("6");
