...
public class MapPaymentInputDetails extends DatabaseService
{
	...
	{
		EBWLogger.trace(this, "Starting Internal/External/Check Transfer On Load Event"); 
		...
		try 
		{
			...
			for(int i=0;i<toObjects.length;i++){
				if(toObjects[i] instanceof DsOnloadAccDetailsTO){
					objDsOnloadAccDetailsTO = (DsOnloadAccDetailsTO)toObjects[i];
				}
				if(toObjects[i] instanceof ArrayList){
					objEligibleAccInfo = (ArrayList)toObjects[i];
				}
			}
			objLoadPaymentDetails.put("ExtAccKeyClientIds",objDsOnloadAccDetailsTO);
	...
	{
		EBWLogger.trace(this, "Starting Internal/External/Check Transfer On Account selection Event"); 
		...
		try 
		{
			...
			MSAcntPlatingDetails objMSAcntPlatingDetails = new MSAcntPlatingDetails();
			...
			for(int i=0;i<toObjects.length;i++){
				...
				if(toObjects[i] instanceof MSAcntPlatingDetails){
					objMSAcntPlatingDetails = (MSAcntPlatingDetails)toObjects[i];
				}
			...
			objOnAccSelectDetails.put("PaymentDetails",objPaymentDetails);
			objOnAccSelectDetails.put("MSUserDetails",objMSDetails);
			objOnAccSelectDetails.put("MSFromAccDetails",objFromMSAcc_Details);
			objOnAccSelectDetails.put("MSToAccDetails",objToMSAcc_Details);
			objOnAccSelectDetails.put("MSAcntPlatingInDetails",objMSAcntPlatingDetails);
	...
	{
		EBWLogger.trace(this, "Starting Internal/External/Check Transfer Submit Event");
	...
	public HashMap setPaymentConfirmDetails(Object toObjects[],ServiceContext serviceContext) throws Exception
	{
		...
		try 
		{
			...
			if(transferType!=null && transferType.equalsIgnoreCase(TxnTypeCode.ACH_WITHDRAWAL) || transferType.equalsIgnoreCase(TxnTypeCode.ACH_DEPOSIT)){
				...
				objgetChannelPaymentDetails.getCheckReqDetails(objEditConfirmPaymentDetails, serviceContext);
			}
		} 
		catch (Exception exception){
			throw exception;
