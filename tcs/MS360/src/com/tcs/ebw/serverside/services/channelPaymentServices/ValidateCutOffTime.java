	...
	public void checkCutOffTime_Info(HashMap txnDetails,ServiceContext serviceContext) throws Exception,SQLException
	{
		...
		try 
		{
			...
			if(currentTime.after(cutOffTime))
			{
				...
				Date nextTransactionDate = MSCommonUtils.evaluateNextTxnDate(repeatValue,businessDate,businessDate,transferType,serviceContext);
				...
				if(transferType!=null && (transferType.equalsIgnoreCase(TxnTypeCode.INTERNAL))){
					...
					estimatedArrDate = MSCommonUtils.calculateEstArrivalDate(nextTransactionDate,transferType,serviceContext); 
				}

				//Updating the payment details information...
				objPaymentDetails.setRequestedDate(ConvertionUtil.convertToAppDateStr(nextTransactionDate));
				objPaymentDetails.setEstimatedArrivalDate(ConvertionUtil.convertToAppDateStr(estimatedArrDate));
	...
	public void checkCutOffTime_Error(HashMap txnDetails,ServiceContext serviceContext) throws Exception,SQLException
	{
		...
		try 
		{
			...
			if(currentTime.after(cutOffTime))
			{
				...
				//Mapping the error code based on the transaction event...
				String cutOffTimeCode = "CutOffTime_Err004";
				if(objPaymentDetails.isTxnApproved() || objPaymentDetails.isTxnRejected()){
					cutOffTimeCode = "CutOffTime_Err002";
				}
				else if((objPaymentDetails.isCheckPrinted() || objPaymentDetails.isCheckVoided())){
					cutOffTimeCode = "CutOffTime_Err003";
				}
				else if(objPaymentDetails.isTxnCancelled()){
					cutOffTimeCode = "CutOffTime_Err005";
				}
