	...
	{ 
		...
		try 
		{
			...
			else 
			{
				...
				//Cancellation, Suspend, Approve case....
				//In case of Approve, Limit should be called only on System Rejected case for delete case...
				if(objPaymentDetails.isTxnCancelled() || objPaymentDetails.isTxnRejected() || objPaymentDetails.isParentTxnSuspended() || objPaymentDetails.isTxnApproved()){
					if(current_owner_role!=null && current_owner_role.equalsIgnoreCase(InitiatorRoleDesc.Client)){
						delete_limit = 1;
						limit_action = Bus_Rule_Input_Desc.Cancel;
						isLimitCallReq = true;
				...
				//Cancellation case....
				if(objPaymentDetails.isTxnResumed()){
					if(current_owner_role!=null && current_owner_role.equalsIgnoreCase(InitiatorRoleDesc.Client)){
						create_limit = 1;
						limit_action = Bus_Rule_Input_Desc.Confirm;
						isLimitCallReq = true;
