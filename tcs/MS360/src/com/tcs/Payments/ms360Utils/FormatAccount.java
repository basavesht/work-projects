...
public class FormatAccount 
{
	...
	public static String getDebitAccountDisp(HashMap txnDetails)
	{
		...
		if(transfer_type!=null && (transfer_type.equalsIgnoreCase(TxnTypeCode.INTERNAL) || transfer_type.startsWith(ChkReqConstants.CHK) || transfer_type.startsWith(TxnTypeCode.PORTFOLIO_LOAN))){
	...
	public static String getCreditAccountDisp(HashMap txnDetails)
	{
		...
		if(transfer_type!=null && transfer_type.equalsIgnoreCase(TxnTypeCode.INTERNAL)){
			...
			accountDisplay.append(MSSystemDefaults.LOAN_ACC_PREFIX_TEXT);
			accountDisplay.append(" ");
			accountDisplay.append(objLoanAcntDetails.getLoanAccount().substring(0,3));
			accountDisplay.append("-");
			accountDisplay.append(objLoanAcntDetails.getLoanAccount().substring(3));
		}
		return accountDisplay.toString();
	}
