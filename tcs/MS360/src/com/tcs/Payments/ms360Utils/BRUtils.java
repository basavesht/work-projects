 ...
 *  224703		   15-06-2011	     P3				 Third Party Indicator Inclusion.
...
public class BRUtils {
	...
	public static String getBRTxnTypeValue(String txn_Type)
	{
		...
		else if(txn_Type.startsWith(TxnTypeCode.PORTFOLIO_LOAN)){
	...
	/**
	 * Extracting the Spoke To Type to be mapped to business rule...
	 * @throws Exception 
	 */
	public static String getSpokeToType(HashMap txnDetails) throws Exception
	{
		String spokeToType = null;
		try 
		{
			...
			//BR Action...
			String eventId = objPaymentDetails.getEventDesc();
			if(eventId!=null && (eventId.equalsIgnoreCase(Bus_Rule_Input_Desc.Create_Confirm) 
					|| eventId.equalsIgnoreCase(Bus_Rule_Input_Desc.Modify_Confirm))){

				String auth_mode = objPaymentDetails.getAuth_mode();
				String otherAcntOwner = objPaymentDetails.getOtherAcntOwner();
				if(auth_mode!= null && !auth_mode.trim().equalsIgnoreCase(""))
				{
					spokeToType = Bus_Rule_Input_Desc.SPOKE_TO_TYPE_PN;
					if(otherAcntOwner!=null && !otherAcntOwner.trim().equalsIgnoreCase("")){
						spokeToType = Bus_Rule_Input_Desc.SPOKE_TO_TYPE_OTHER;
					}
				}
			}
		}
	...
	/**
	 * Extracting the Spoke To Name as selected in the pre confirmation screen....
	 * @throws Exception 
	 */
	public static String getSpokeToName(HashMap txnDetails) throws Exception
	{
		String spokeToName = null;
		try 
		{
			//Mapping the payment detail object with the Payment details...
			PaymentDetailsTO objPaymentDetails = new PaymentDetailsTO();
			...
			if(eventId!=null && (eventId.equalsIgnoreCase(Bus_Rule_Input_Desc.Create_Confirm) 
				...
				//Hashed unique id of the account owner as selected on the PreConfrim screen...
				String spokeToId = objPaymentDetails.getVerbal_auth_client_name();

				//Extract Debit account plating details...
				ArrayList authorisedEntities = new ArrayList();
				AccountPlating acntPlating = MSCommonUtils.extractDebitAcntPlatingDtls(txnDetails);
