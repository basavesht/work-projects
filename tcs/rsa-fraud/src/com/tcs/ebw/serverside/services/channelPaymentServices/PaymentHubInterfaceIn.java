...
import com.tcs.Payments.EAITO.MO_INP_CANCEL_TXN;
...
public class PaymentHubInterfaceIn {
	 ...
	 * @throws Exception
	 */
	public static Vector setPaymentHubParams_CancelTxn(HashMap txnDetails,ServiceContext serviceContext) throws Exception
	{
		EBWLogger.trace("PaymentHubInterfaceIn", "Setting the Payments Hub required input details...");
		Vector paymentHubInputVec = new Vector();
		try 
		{
			...
			//Input Mappings...
			MO_INP_CANCEL_TXN objCancelTxnInput = new MO_INP_CANCEL_TXN();
			objCancelTxnInput.setPYMNT_POS_NUM(objPaymentDetails.getPaymentHubTxnId());
			objCancelTxnInput.setPAYPERF(objPaymentDetails.getTransactionId());
			objCancelTxnInput.setTXN_TYPE(objPaymentDetails.getTransfer_Type());

			//Adding the Payments Hub attributes...
			paymentHubInputVec.add(objCancelTxnInput);
		} 
		catch (Exception exception){
			throw exception;
		}
