 ...
 * **********************************************************
 */
public class UpdateTxnParentService extends DatabaseService {

	public UpdateTxnParentService(){

	}

	/**
	 * Creating the next child transaction and updating the recurring details in the 
	 * DS_PAY_TXNS & TXN_PARENT Table after modifying or creating the transactions..
	 * @param statusFlag
	 * @param txnDetails
	 ...
	 * @throws SQLException
	 */
	public void setUpdateTxnParentDetails(HashMap txnDetails,ServiceContext serviceContext)throws Exception,SQLException 
	{
		EBWLogger.trace(this, "Starting setUpdateTxnParentDetails method in UpdateTxnParentService class");
		try
		{
			...
			nextBusinessDate = MSCommonUtils.evaluateNextTxnDate(frequencyValue,last_transfer_date,parentTxnDate,transferType,serviceContext);
			pref_next_business_dt = MSCommonUtils.calculatePrefNextTxnDate(frequencyValue,last_transfer_date,parentTxnDate,transferType);
			...
			objUpdateTxnParent.setAccum_amt(ConvertionUtil.convertToBigDecimal(objPaymentDetails.getTransfer_Amount()));
			objUpdateTxnParent.setAccum_txn_no(new Double(1D));
			objUpdateTxnParent.setLast_txn_id(objPaymentDetails.getTransactionId());
			objUpdateTxnParent.setLast_txn_dt(ConvertionUtil.convertToTimestamp(objPaymentDetails.getRequestedDate()));
			...
			objUpdateTxnParent.setActual_previous_txn_dt(ConvertionUtil.convertToTimestamp(objPaymentDetails.getRequestedDate()));
			...
			nextTxnPaymentTO.setFriendlyname_from(null);
			nextTxnPaymentTO.setNickname_from(null);
			...
			nextTxnPaymentTO.setFriendlyname_to(null);
			nextTxnPaymentTO.setNickname_to(null);
			...
			nextTxnPaymentTO.setRetirement_mnemonic(objPaymentDetails.getRetirement_mnemonic());
			nextTxnPaymentTO.setQualifier(objPaymentDetails.getQualifier());
			nextTxnPaymentTO.setReverse_qualifier(objPaymentDetails.getReverse_qualifier());
			...
			createNextTxnInstance(txnDetails,nextTxnBatchTO,nextTxnPaymentTO,objUpdateTxnParent,serviceContext);
	...
	public void updateTxn_ParentDetails(HashMap txnDetails,TxnParentTO objUpdateTxnParent,ServiceContext serviceContext) throws Exception
	{
		...
		try 
		{
			...
			String updateTxnPayStmntId="updateParentTxn";
			int parentTxnCurStatus = 0; 

			//Resetting the accumulated amount and no of transfers based on the parent current transaction status...
			if(objPaymentDetails.getTxnCurrentStatusCode()!=null){
				parentTxnCurStatus = new Integer(objPaymentDetails.getTxnCurrentStatusCode()).intValue();
				List<Integer> validTxnStatuses = new ArrayList<Integer>(Arrays.asList(4,103));
				if(!validTxnStatuses.contains(parentTxnCurStatus)) {
					objUpdateTxnParent.setAccum_amt(new BigDecimal(0D));
					objUpdateTxnParent.setAccum_txn_no(new Double(0D));
				}
			}
			...
			EBWLogger.logDebug(this,"Executing Query : "+updateTxnPayStmntId);
			EBWLogger.logDebug(this,"Input Parameters mapped"+objUpdateTxnParent);
			execute(updateTxnPayStmntId, objUpdateTxnParent, isTxnCommitReq);
			EBWLogger.logDebug(this,"Execution Completed.... "+updateTxnPayStmntId);

			//Updating the accumulated transfer, accumulated amount and calculated next_transfer_date...
			objPaymentDetails.setAccumulated_amount(objPaymentDetails.getAccumulated_amount().add(objUpdateTxnParent.getAccum_amt()));
			objPaymentDetails.setAccumulated_transfers(objPaymentDetails.getAccumulated_transfers().doubleValue()+ objUpdateTxnParent.getAccum_txn_no().doubleValue());
	 ...
	 * @throws Exception
	 */
	public void createNextTxnInstance(HashMap txnDetails,DsBatchTO nextTxnBatchTO,DsPayTxnsTO nextTxnPaymentTO,TxnParentTO objUpdateTxnParent,ServiceContext serviceContext) throws Exception
	{
		EBWLogger.trace(this, "Setting the input details for updating the recurring transfer details in case of Skip Trasnfers..");
		try 
		{
			...
			//Status mappings based on Parent transaction status(Default to 6:Scheduled)...
			String childTxnPaymentStatus = "6";
			if(objPaymentDetails.isParentTxnSuspended()){
				childTxnPaymentStatus = "46";
			}
			...
			if(isRecurringActive && (dont_spawn_flag==null || dont_spawn_flag.equalsIgnoreCase("N")))
			{
				for (int i= 0; i< nextChildTxnTOobj.length;i++)  
				{
					if(nextChildTxnTOobj[i] instanceof com.tcs.ebw.payments.transferobject.DsBatchTO){
						((DsBatchTO)nextChildTxnTOobj[i]).setBatstatus(childTxnPaymentStatus);
					}
					if(nextChildTxnTOobj[i] instanceof com.tcs.ebw.payments.transferobject.DsPayTxnsTO){
						...
						((DsPayTxnsTO)nextChildTxnTOobj[i]).setPayccsstatuscode(childTxnPaymentStatus);
			...
			for(int j=0;j<merlinAccDetails.size();j++){
				if(merlinAccDetails.get(j) instanceof MerlinOutResponse){
					...
					if(accMerlinResponse.getAccountNumber().equalsIgnoreCase(dsPayTxnsTO.getPaydebitaccnum())){
						if(dsPayTxnsTO.getTxn_type().equalsIgnoreCase(TxnTypeCode.INTERNAL) || dsPayTxnsTO.getTxn_type().startsWith(ChkReqConstants.CHK)){
	...
	{	
		try 
		{
			...
			if(nextTxnPaymentTO!=null && nextTxnPaymentTO.getPaypayref()!=null){
				...
				objPaymentDetails.setChildTxnRequestedExecDate(ConvertionUtil.convertToAppDateStr(nextTxnPaymentTO.getRequested_exe_dt()));
